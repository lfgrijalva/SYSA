...
if ($user != "") {
	...
	//when user submits
		...
		$gned = intval(trim($_POST["gned"]));
		$itce = intval(trim($_POST["itce"]));
		$netd = intval(trim($_POST["netd"]));
		$oop = intval(trim($_POST["oop"]));
		$syde = intval(trim($_POST["syde"]));
		$sysa = intval(trim($_POST["sysa"]));
		$webd = intval(trim($_POST["webd"]));
		$gpa = "";
		...
		//gned
		if (!isset($gned) || $gned == '') //Is the input empty?
		{
		...
		//itce
		if (!isset($itce) || $itce == '') //Is the input empty?
		{
			$error .= "ITCE field cannot be empty <br/>";
		} elseif (!is_numeric($itce)) //is the input numeric?
		{
			$error .= "<br/>Your input for ITCE must be numeric. You entered: " . $gned .
				" \nPlease Try Again! Example: 88<br/>";
			$itce = '';
		...
		//netd
		if (!isset($netd) || $netd == '') //Is the input empty?
		{
		...
		//oop
		if (!isset($oop) || $oop == '') //Is the input empty?
		{
		...
		//syde
		if (!isset($syde) || $syde == '') //Is the input empty?
		{
		...
		//sysa
		if (!isset($sysa) || $sysa == '') //Is the input empty?
		{
		...
		//webd
		if (!isset($webd) || $webd == '') //Is the input empty?
		{
		...
		if ($error == '') {
			...
			echo "<p>A record was added to the database<p/>";
		} else {
			echo "<h1>Errors were encountered</h1><hr/><br/> " . $error;
		}
	};
} else {
